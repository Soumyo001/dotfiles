...
echo "[1/16] Creating backup directories..."
mkdir -p "$BACKUP_DIR"/{i3,polybar,picom,rofi,gtk,x11,scripts,vesktop,wallpapers,fontconfig,browsers,kitty,dconf,redshift,zsh}
echo "Directory structure created at $BACKUP_DIR."

echo "[2/16] Backing up i3 config..."
if [ -d ~/.config/i3 ]; then
...
echo "[5/16] Backing up Fontconfig..."
...
echo "[6/16] Backing up GTK configs..."
gtk2_ok=false
gtk3_ok=false
gtk4_ok=false
...
echo "[7/16] Backing up GNOME font keys (dconf/gsettings)..."
...
echo "[8/16] Backing up X11 settings..."
...
echo "[9/16] Backing up custom scripts..."
...
echo "[10/16] copying vesktop themes"
...
echo "[13/16] Copying redshift config (if have any)..."
...
echo "[14/16] Copying kitty config..."
